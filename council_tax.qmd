...
Below are some scripts to download, clean, and visualize council tax data for England and Wales. The data includes average council tax rates by band for the year 2025-26, and the script calculates estimated council tax income for each LSOA based on these rates.

## Download Council Tax Band D Data from the Government Website for 2025-26

Table 8a is the one we want, which includes the average council tax for Band D properties, both including and excluding the adult social care precept.

Get the data from here <https://www.gov.uk/government/statistics/council-tax-levels-set-by-local-authorities-in-england-2025-to-2026>
...
# Step 4: Define multipliers to generate other bands
...
## now convert all of the band columns to numbers rather than named numbers

table8a_wide <- table8a_wide %>%
  mutate(across(starts_with("band_"), ~ unname(.)))

# Step 6: Save to CSV
#write.csv(df_bands, "council_tax_bands_2025_26.csv", row.names = FALSE)
#write_csv(table8a_wide, "data/table8a_wide.csv")
...
Some more data here: <https://www.gov.uk/government/statistical-data-sets/live-tables-on-council-tax>

OK, below, the stock of properties came from here:

<https://www.gov.uk/government/statistics/council-tax-stock-of-properties-2024>

```{r}
### We Don't need to run this every time so set to false for now
#| eval: false
#| include: false
...
stock_2024 <- read_csv(file = file_path, na = c("N/A", "-", ".."))
write_csv(stock_2024, "data/stock_2024.csv")
...
write_csv(lsoa_enriched, "data/lsoa_enriched.csv")
...
lsoa_enriched <- as.data.frame(lsoa_enriched)

#write_csv("data/nested.csv", stock_2024)
```

```{r}
unmatched_codes <- lsoa_nested %>%
  anti_join(table8a_wide, by = c("laua_code" = "ons_code")) %>%
  select(laua_code) %>%
  distinct()
```
